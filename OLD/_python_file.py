...
'''
@alt(ディレクトリ|フォルダ)
filepathのディレクトリ名が欲しい
filepathからディレクトリ名[を得る|取り出す]
'''

os.path.abspath(filepath)
'''
filepathの絶対[|ファイル]パスが欲しい
...
'''
スクリプトファイルの[絶対|]パスが欲しい
'''

os.path.dirname(os.path.abspath(__file__))
'''
スクリプトファイルのディレクトリ[名|パス]が欲しい
'''

os.path.join(os.path.dirname(os.path.abspath(__file__)), filepath)
'''
スクリプトファイルと同じディレクトリのfilepathのパスが欲しい
'''
...
'''
@test($$;encoding)
option: エンコーディングを__Y__に設定する
option: __Y__を使う
'''

buffering = 0
'''
option: [バッファリングを無効にする|バッファを使わない]
'''
...
buffering = 4096
'''
option: [バッファリング|バッファ]のサイズを[設定する|大きくする|小さくする]
'''

errors = 'strict'
'''
option: エラーがあるとき、例外を発生させる[ように設定する|]
'''

errors = 'ignore'
'''
option: エラーを無視する[ように設定する|]
'''
