...
def keyword(s):
    ...
    for t in janome.tokenize(s):
        ...
        #print(base, pos)
        if pos == '助詞' or pos == '記号' or pos == '助動詞':
            ss.append('/')
            continue
        if pos == '形容詞':
            base = f'{t.surface}'
        if pos == '名詞' and base == 'さ':
            base = f'{base}/'
        if base not in ss:
            ss.append(base)
    return [t for t in ''.join(ss).split('/') if t != '']


# @poly(__要素__;_文字列_;_文字列_,".csv")
# @poly(__要素__/_文字列_/セミコロン ";")
...
#python3 make_train.py _*.py new_file.jsonl
